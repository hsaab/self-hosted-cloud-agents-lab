...
data:
  scale.py: |
    ...
    if connected <= 0:
      ...
      if active >= connected and current_replicas < max_replicas:
    ...
    utilization = 0.0 if connected <= 0 else active / connected * 100
    print(
      "cursor worker autoscale: "
      f"connected={connected:.0f} active={active:.0f} "
